...
// --- STELLA DERPY STYLE with direction ---
// ONLY mascot art is changed from Porkchop. Environment/movement below remains donor logic.
    ...
    if (!transitioning && !grassMoving && !pendingGrassStart) {
        ...
        if (now - lastFlipTime > flipInterval) {
            ...
            if (walkRoll < 50) {
                ...
                if (onRightSide) {
                    targetX = random(85, 108);
                } else {
                    targetX = random(20, 45);
                }
            } else {
                facingRight = !facingRight;
                lastFlipTime = now;
            ...
            if (abs(targetX - currentX) > 15) {
                ...
                lastFlipTime = now;
                if (random(0, 4) == 0) {
                    flipInterval = random(15000, 30000);
                } else {
